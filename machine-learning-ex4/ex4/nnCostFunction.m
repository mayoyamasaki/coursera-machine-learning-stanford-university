...
% y is 5000, 1
% X is 5000, 400
X = [ones(m, 1) X]; % add bias, X is 5000, 401
% Theta1 is 25, 401
% Theta2 is K, 26
for i = 1:m
    A1 = X(i, :); % 1, 401
...
R1 = ones(size(Theta1_grad));
R1(:, 1) = zeros(size(R1, 1), 1);
R2 = ones(size(Theta2_grad));
R2(:, 1) = zeros(size(R2, 1), 1);
Theta1_grad += R1 .* (lambda/m * Theta1);
Theta2_grad += R2 .* (lambda/m * Theta2);
